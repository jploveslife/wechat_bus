...
public class RedisTool {
    ...
    public long incr(String key){
        return redisTemplate.opsForValue().increment(key, incrCount);
    }

    /**
     * 递增
     * @param key 键
     * @return delta 要增加几(大于0)
     */
    public long incr(String key, int time){
        Long val = redisTemplate.opsForValue().increment(key, incrCount);
        if(val == 1L && time > 0){
            expire(key,time);
        }
        return val;
